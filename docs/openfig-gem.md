...
This file documents the current persistence model used by Recast, based on the domain types in `app/core/types.ts` and the SQLite schema and repository behavior in `app/database/store.ts`.

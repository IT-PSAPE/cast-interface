import type { ReactNode } from 'react';
import { Grid2x2, List } from 'lucide-react';
import { SegmentedControl } from '@renderer/components/controls/segmented-control';
import { useResourceDrawer } from './resource-drawer-context';

interface ViewOption {
  mode: 'grid' | 'list';
  label: string;
  icon: ReactNode;
}

export function ResourceDrawerModeControl() {
  const { drawerViewMode, setDrawerViewMode } = useResourceDrawer();

  const viewOptions: ViewOption[] = [
    { mode: 'grid', label: 'Grid view', icon: <Grid2x2 size={14} strokeWidth={1.5} /> },
    { mode: 'list', label: 'List view', icon: <List size={14} strokeWidth={1.5} /> },
  ];

  function handleValueChange(nextValue: string | string[]) {
    if (Array.isArray(nextValue)) return;
    if (!isResourceDrawerViewMode(nextValue)) return;
    setDrawerViewMode(nextValue);
  }

  function renderViewItem(option: ViewOption) {
    return (
      <SegmentedControl.Icon key={option.mode} value={option.mode} title={option.label} aria-label={option.label}>
        {option.icon}
      </SegmentedControl.Icon>
    );
  }

  return (
    <SegmentedControl value={drawerViewMode} onValueChange={handleValueChange} aria-label="Resource drawer mode">
      {viewOptions.map(renderViewItem)}
    </SegmentedControl>
  );
}

function isResourceDrawerViewMode(value: string): value is ViewOption['mode'] {
  return value === 'grid' || value === 'list';
}

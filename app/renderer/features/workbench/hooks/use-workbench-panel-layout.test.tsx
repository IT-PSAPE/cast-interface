...
describe('useWorkbenchPanelLayout', () => {
  ...
  it('restores persisted layout per view', () => {
    ...
    act(() => {
      ...
      firstProbe.current?.togglePanel('slideEditor', 'left');
      firstProbe.current?.togglePanel('slideEditor', 'bottom');
      firstProbe.current?.togglePanel('overlayEditor', 'right');
    });
    ...
    expect(secondProbe.current?.panelVisibility.slideEditor.left).toBe(false);
    expect(secondProbe.current?.panelVisibility.slideEditor.bottom).toBe(false);
    expect(secondProbe.current?.panelVisibility.overlayEditor.right).toBe(false);
  ...
  it('keeps overlay visibility independent from edit visibility', () => {
    ...
    act(() => {
      probe.current?.togglePanel('slideEditor', 'left');
    });

    expect(probe.current?.panelVisibility.slideEditor.left).toBe(false);
    expect(probe.current?.panelVisibility.overlayEditor.left).toBe(true);

    act(() => {
      probe.current?.togglePanel('overlayEditor', 'left');
    });

    expect(probe.current?.panelVisibility.slideEditor.left).toBe(false);
    expect(probe.current?.panelVisibility.overlayEditor.left).toBe(false);
  });
});

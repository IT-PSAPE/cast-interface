...
export function ShowPreviewRail() {
  ...
  return (
    <aside className="grid h-full min-h-0 grid-rows-[auto_auto_1fr] overflow-hidden border-l border-stroke bg-surface-1">
      ...
      <div className="border-b border-stroke p-2">
        <ActionButton variant="danger" onClick={handleClearAll} className="w-full text-center">
          Clear All Layers
        </ActionButton>
...
function LayerClearButton({ active, label, icon, onClick }: LayerClearButtonProps) {
  ...
  return (
    <ActionButton variant="ghost" onClick={onClick} className={`grid h-8 w-8 place-items-center p-0 ${stateClass}`}>

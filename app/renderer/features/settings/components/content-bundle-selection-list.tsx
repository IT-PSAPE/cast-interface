...
function ContentBundleSelectionRow({ item, selected, onToggle }: ContentBundleSelectionRowProps) {
  ...
  return (
    <SelectableRow
      selected={selected}
      onClick={handleClick}
      leading={<ContentItemIcon entity={item} size={14} strokeWidth={1.75} className="text-text-tertiary" />}
      title={item.title}
      trailing={<span className="shrink-0 text-xs uppercase tracking-wide text-text-tertiary">{item.type}</span>}
      className="h-9"
    />
  );
}
...
export function ContentBundleSelectionList({ items, selectedIds, onToggle }: ContentBundleSelectionListProps) {
  ...
  if (rows.length === 0) {
    return <div className="rounded border border-border-primary bg-background-tertiary/30 px-3 py-4 text-sm text-text-tertiary">No matching items.</div>;
  }

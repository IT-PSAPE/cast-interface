...
export function ImportExportSettingsPanel() {
  ...
  return (
    <div className="grid gap-6">
      ...
      >
        {!hasInspection ? (
          <div className="rounded border border-border-primary bg-background-tertiary/30 px-3 py-4 text-sm text-text-tertiary">
      ...
      {state.message ? (
        <div className="rounded border border-border-primary bg-background-tertiary/25 px-3 py-2 text-sm text-text-secondary">
          {state.message}
        </div>
      ) : null}
    </div>
...
function renderInspectionContent({
  ...
  return (
    <div className="grid gap-4">
      <div className="grid gap-1 rounded border border-border-primary bg-background-tertiary/25 p-3">
        <div className="text-sm font-medium text-text-primary">{importPath}</div>
      ...
      {inspection.items.length > 0 ? (
        <div className="grid gap-1 rounded border border-border-primary bg-background-tertiary/25 p-3">{inspectionItemRows}</div>
      ) : null}
      ...
      {blockedImportReasons.length > 0 ? (
        <div className="grid gap-1 rounded border border-border-primary bg-background-tertiary/25 p-3 text-xs text-text-tertiary">{blockedReasonRows}</div>
      ) : null}
    </div>
  );
}
...
function renderInspectionItemRow(item: { id: string; title: string; type: string; slideCount: number }) {
  return (
    <div key={item.id} className="flex items-center justify-between gap-3 text-sm text-text-secondary">

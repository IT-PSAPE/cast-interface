...
export function OverlayEditorLayersPanel() {
  ...
  if (orderedElements.length === 0) {
    return (
      <EmptyState.Root data-ui-region="object-list-panel">
        <EmptyState.Title>No objects in this overlay.</EmptyState.Title>
      </EmptyState.Root>
    );
  }
  ...
  return (
    <div data-ui-region="object-list-panel" className="flex w-full flex-col gap-1.5">
      {orderedElements.map((element) => (
        <OverlayLayerRow key={element.id} element={element} isSelected={element.id === selectedElementId} onSelect={selectElement} onSetInspectorTab={setInspectorTab} />
      ))}
    </div>
  );
}

function OverlayLayerRow({
  element,
  isSelected,
  onSelect,
  onSetInspectorTab,

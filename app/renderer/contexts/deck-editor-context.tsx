...
const DeckEditorContext = createContext<SlideEditorContextValue | null>(null);

export function DeckEditorProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    if (previousWorkbenchMode !== 'deck-editor' || workbenchMode === 'deck-editor') return;
  ...
  return <DeckEditorContext.Provider value={value}>{children}</DeckEditorContext.Provider>;
}

export function useDeckEditor(): SlideEditorContextValue {
  const context = useContext(DeckEditorContext);
  if (!context) throw new Error('useDeckEditor must be used within DeckEditorProvider');
  return context;
}
